...
%               Connecter la boite de controle du shutter sur PORT1 a
%               l'arriere du SR400 
...
%     initialise_SR400(23) %l'addresse par defaut est 23
    SR400 = initialise(23) %l'addresse par defaut est 23
end
...
case {'INITIALISE'}
    ...
    SR400 = initialise(23)
    sr400('CONFIG_INITIALE')
case 'CONFIG_INITIALE'
    ecrire(['IFC  DCL UNT UNL REN MTA LISTEN 23 SPE SDC']) %Clear SR400
...
case 'CI'
    ...
    if nargin==2 %lecture
        %vider_scrap_compteur;
        out = str2num(query([fonction counter]))
...
%         out = str2num(gpib('rd', SR400));%lit l'input du compteur
        ...
        ecrire([fonction counter ',' input_counter])
...
%             out = gpib('rd', SR400);%lit l'input du compteur
            ...
            ecrire([fonction counter ',' nouvel_argument])
...
case {'EA','EB'}
    ...
    ecrire([fonction char(13) char(10)]);
%     gpib('wrt', SR400, [fonction char(13) char(10)]);
    for i=1:N %lit les points un a un
        pause(.01)
        out(i) = str2num(lire);%lit les resultat de chaque mesure
...
case {'NP','TL'}
    ...
    if nargin > 1 %controle
        nouvel_argument = varargin{1};
        if isnumeric(nouvel_argument); nouvel_argument=num2str(nouvel_argument); end   
        ecrire([fonction ' ' nouvel_argument char(10)])
%         sta = gpib('wrt', SR400, [fonction ' ' nouvel_argument char(10)]);
        varargout{1} = sr400(fonction);
    else %lecture
        out = query([fonction char(10)]);
%         sta = gpib('wrt', SR400, [fonction char(10)]);
%         out = gpib('rd', SR400);
        if double(out(end)) == 10; out = out(1:end-1); end
        if double(out(end)) == 13; out = out(1:end-1); end
...
global SR400
fprintf(SR400, chaine); 

function out = lire
global SR400
delaiLecture = 0.2;
%global debugSP275
alire = SR400.BytesAvailable;
attendre = 1;
while attendre
    pause(delaiLecture)
    if SR400.BytesAvailable > alire
        alire = SR400.BytesAvailable;
    else
        attendre = 0;
    end
...
% end
%
%INSTRUMENT = gpib('dev', 0, addresse, 0, 13, 1, 0);
%ibsta = gpib('wrt', INSTRUMENT, 'ck'); %initialiser le discriminateur/compteur de photons - SR400
